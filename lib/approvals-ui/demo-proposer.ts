...
function lastGate(policy: ApprovalPolicy): ApprovalStep | undefined {
  const approvedIds = new Set(
    policy.steps.filter(isTerminalStep).filter((t) => t.outcome === "approved").map((t) => t.id),
  )
  const gates = policy.steps.filter(isApprovalStep)
...
export const demoProposer: Proposer = async (instruction, policy) => {
  ...
  if (threshold) {
    ...
    if (!Number.isFinite(value) || role.length === 0) {
      return clarifyProposal(policy, "I could not read the amount or the role in that instruction.")
    }
  ...
  if (add) {
    ...
    return runOrExplain(policy, [{ op: "add-approver", stepId: step.id, approver: { name, title } }], `Add ${name} to "${step.label}".`)
  }
  ...
  if (removeFrom) {
    ...
    return runOrExplain(policy, [{ op: "remove-approver", stepId: step.id, name: match.name }], `Remove ${match.name} from "${step.label}".`)
  }
  ...
  if (remove) {
    ...
    if (!step) return clarify as EditProposal
    return proposeEdits(policy, [{ op: "remove-step", stepId: step.id }], `Remove "${step.label}" and rewire around it.`)
  }
