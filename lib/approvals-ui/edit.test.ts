...
describe("applyEditOp", () => {
  ...
  it("remove-step rewires edges and roots", () => {
    ...
    const withoutMiddle = applyEditOp(examplePolicy, {
      op: "remove-step",
      stepId: "director-review",
    });
...
describe("proposeEdits", () => {
  it("passes clarify through without mutating", () => {
    const proposal = proposeEdits(examplePolicy, [{ op: "clarify", question: "Which step?" }]);
...
describe("demoProposer", () => {
  ...
  it("sets a quorum", async () => {
    const proposal = await demoProposer(
      "Require 3 of the approvers on Finance review",
      examplePolicy
    );
